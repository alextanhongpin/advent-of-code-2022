package main

import (
	"fmt"
	"log"
	"regexp"
	"strconv"
	"strings"
)

var re = regexp.MustCompile(`(\d+|R|L)`)

func main() {
	fmt.Println(part1(input1))
	fmt.Println(part1(input2))
	fmt.Println(part2(input2))
}

type direction complex128

func (d direction) clockwise() direction {
	d = d * -1i
	return d
}

func (d direction) anticlockwise() direction {
	d = d * 1i
	return d
}

func (d direction) inv() complex128 {
	return complex(real(d), -imag(d))
}

func (d direction) isUp() bool {
	return imag(d) > 0
}
func (d direction) isDown() bool {
	return imag(d) < 0
}
func (d direction) isLeft() bool {
	return real(d) < 0
}
func (d direction) isRight() bool {
	return real(d) > 0
}

func (d direction) complex() complex128 {
	// NOTE: The imaginary number treats going down as -tive, but in our scenario
	// going down is increasing y.
	return complex(real(d), -imag(d))
}

func part1(input string) int {
	grid, ins := parse(input)

	boundary := grid.bound()
	dir := direction(1 + 0i) // Facing right.
	start := grid.topLeft()

	for _, in := range ins {
		switch in {
		case "R":
			//fmt.Println("turn cw")
			dir = dir.clockwise()
		case "L":
			//fmt.Println("turn anti-cw")
			dir = dir.anticlockwise()
		default:
			n := toInt(in)
			for i := 0; i < n; i++ {
				p := start + dir.complex()
				if grid[p].valid() {
					if grid[p].wall() {
						break
					}

					start = p
					continue
				}

				// Out of boundary.
				switch {
				case dir.isUp():
					// Start from bottom.
					p = complex(real(p), imag(boundary))
				case dir.isDown():
					// Start from top.
					p = complex(real(p), 0)
				case dir.isLeft():
					p = complex(real(boundary), imag(p))
				case dir.isRight():
					p = complex(0, imag(p))
				}

				for !grid[p].valid() {
					p += dir.complex()
				}

				if grid[p].wall() {
					break
				}

				start = p
			}
		}
	}

	total := (imag(start) + 1) * 1_000 // Row
	total += (real(start) + 1) * 4     // Column
	if dir.isUp() {
		total += 3
	} else if dir.isDown() {
		total += 1
	} else if dir.isLeft() {
		total += 2
	} else if dir.isRight() {
		total += 0
	}

	return int(total)
}

func part2(input string) int {
	grid, ins := parse(input)

	dir := direction(1 + 0i) // Facing right.
	start := grid.topLeft()
	fmt.Println("start", start)

	//         0  1  2
	//0					 [1][2]
	//1					 [3]
	//2				[4][5]
	//3				[6]
	face := func(p complex128) int {
		x, y := int(real(p))/50, int(imag(p))/50
		switch x {
		case 0:
			switch y {
			case 2:
				return 4
			case 3:
				return 6
			}
		case 1:
			switch y {
			case 0:
				return 1
			case 1:
				return 3
			case 2:
				return 5
			}
		case 2:
			if y == 0 {
				return 2
			}
		}
		log.Fatalf("invalid face at: %v", p)
		return 0
	}

	for _, in := range ins {
		switch in {
		case "R":
			dir = dir.clockwise()
		case "L":
			dir = dir.anticlockwise()
		default:
			n := toInt(in)
			fmt.Println("walking steps", n)
			for i := 0; i < n; i++ {
				p := start + dir.complex()
				if grid[p].valid() {
					if grid[p].wall() {
						break
					}

					start = p
					continue
				}
				fmt.Println("current", start, dir, p, face(start), "grid", string(grid[start]))

				/*
						 [1][2]
						 [3]
					[4][5]
					[6]
				*/

				// There are 14 edges that are not connected, we need to link them.
				// 1U -> 6L, cw
				// 6L -> 1U, an
				// 1L -> 4L, cw, cw
				// 4L -> 1L, cw, cw
				// 2U -> 6D, no change
				// 6D -> 2U, no change
				// 2R -> 5R, cw, cw
				// 5R -> 2R, cw, cw
				// 2D -> 3R, cw
				// 3R -> 2D, an
				// 3L -> 4T, an
				// 4T -> 3L, cw
				// 5D -> 6R, cw
				// 6R -> 5D, an
				ndir := dir
				x, y := real(start), imag(start)
				switch face(start) {
				case 1:
					switch {
					case dir.isLeft():
						// 1L -> 4L, cw, cw
						dir = dir.clockwise().clockwise()
						p = complex(0, 149-y)
					case dir.isUp():
						// 1U -> 6L, cw
						dir = dir.clockwise()
						p = complex(0, x+100)
					}
				case 2:
					switch {
					case dir.isDown():
						// 2D -> 3R, cw
						dir = dir.clockwise()
						p = complex(99, x-50)
					case dir.isRight():
						// 2R -> 5R, cw, cw
						dir = dir.clockwise().clockwise()
						p = complex(99, 149-y)
					case dir.isUp():
						// 2U -> 6D, no change
						p = complex(x-100, 199)
					}
				case 3:
					switch {
					case dir.isLeft():
						// 3L -> 4T, an
						dir = dir.anticlockwise()
						p = complex(y-50, 100)
					case dir.isRight():
						// 3R -> 2D, an
						dir = dir.anticlockwise()
						p = complex(y+50, 49)
					}
				case 4:
					switch {
					case dir.isLeft():
						// 4L -> 1L, cw, cw
						dir = dir.clockwise().clockwise()
						p = complex(50, 149-y)
					case dir.isUp():
						// 4T -> 3L, cw
						dir = dir.clockwise()
						p = complex(50, x+50)
					}
				case 5:
					switch {
					case dir.isDown():
						// 5D -> 6R, cw
						dir = dir.clockwise()
						p = complex(49, x+100)
					case dir.isRight():
						// 5R -> 2R, cw, cw
						dir = dir.clockwise().clockwise()
						p = complex(149, 149-y)
					}
				case 6:
					switch {
					case dir.isDown():
						// 6D -> 2U, no change
						p = complex(x+100, 0)
					case dir.isLeft():
						// 6L -> 1U, an
						dir = dir.anticlockwise()
						p = complex(y-100, 0)
					case dir.isRight():
						// 6R -> 5D, an
						dir = dir.anticlockwise()
						p = complex(y-100, 149)
					}
				}
				if grid[p].wall() {
					dir = ndir
					break
				}
				start = p
			}
		}
	}

	total := (imag(start) + 1) * 1_000 // Row
	total += (real(start) + 1) * 4     // Column
	if dir.isUp() {
		total += 3
	} else if dir.isDown() {
		total++
	} else if dir.isLeft() {
		total += 2
	} else if dir.isRight() {
		total += 0
	}

	return int(total)
}

func toInt(s string) int {
	n, err := strconv.Atoi(s)
	if err != nil {
		panic(err)
	}

	return n
}

type tile rune

func (t tile) wall() bool  { return t == '#' }
func (t tile) path() bool  { return t == '.' }
func (t tile) valid() bool { return t.wall() || t.path() }

type grid map[complex128]tile

func (g grid) bound() complex128 {
	var max complex128
	for p := range g {
		if real(p) > real(max) {
			max = complex(real(p), imag(max))
		}
		if imag(p) > imag(max) {
			max = complex(real(max), imag(p))
		}
	}
	return max
}

func (g grid) topLeft() complex128 {
	p := 0 + 0i
	for !g[p].valid() {
		p++
	}
	return p
}

func parse(input string) (grid, []string) {
	parts := strings.Split(input, "\n\n")
	m, ins := parts[0], parts[1]
	grid := make(grid)

	for y, row := range strings.Split(m, "\n") {
		for x, t := range row {
			p := complex(float64(x), float64(y))
			switch t {
			case '.', '#':
				grid[p] = tile(t)
			default:
			}
		}
	}

	return grid, re.FindAllString(ins, -1)
}

var input1 = `        ...#
        .#..
        #...
        ....
...#.......#
........#...
..#....#....
..........#.
        ...#....
        .....#..
        .#......
        ......#.

10R5L5R10L4R5L5`
var input2 = `                                                  ............#..#..................#............#.......#..............#.............................
                                                  ...#...#...#.......#............................#.#.....#..#...........................#......#.....
                                                  #.....#....#....#...............#..#.........#..................#......................#.##......#..
                                                  ...#........#.....#...........#.......#.......###....#.......#.............##...........#.#.........
                                                  ..............#.......#............................#.....#..........#....#.........................#
                                                  ...................#.#........#.......#.......#.........#......................#......#.#...#.....#.
                                                  ......#.#......#....#...........................#..........##..............#................#..#....
                                                  ....#...#..#...............................................................#.............#..........
                                                  .###.#........#.................#............#....................#....................#............
                                                  .........................#....#............#.....#.......#......#........#.....#..........#.........
                                                  ..............#....#.........#...#...#...........#..............#....##................#........#...
                                                  ..............#......#.#...................#.....................................#......#....#......
                                                  ..#......#.#................#........#...........#...#...#....#...............#.....#...............
                                                  .#..#...##................#......#..........................#.#....................#...............#
                                                  ......................#..#............#..#...............#..............##........#..............#..
                                                  ......#..#........#.......#..........#.........#...#.....#............##...#.##...#...............#.
                                                  .......#....#..#.............#.........#...........#........#........#.........#..#...........#..#.#
                                                  ...................#............#...............#.....#.............#.#......................#......
                                                  ...#..........#.................#..........#..#.................#.......#..#....#...#...............
                                                  ...#..##...#.....#.....................##...............#.........##.#.......###......###.#.#.##....
                                                  .............#.....#..............#...............#..........#.............................##.......
                                                  ...#......#....#.............................#..................#.#....#...............#....#.......
                                                  ....#..................#................#......#..................................#..##...#.........
                                                  ...#.......#............##...#..#...#...##.....#..#....................................#............
                                                  .#.....#...#..........................#.........................................#.#......#........#.
                                                  .........#.......................###..........#..............................#......#..##...........
                                                  ..................#.#...#.#..............##...................#........#....................#.#.....
                                                  ........#...........................#.........#.....#...#......................#..........#.........
                                                  ..........#........#........#.........................#................#.................#......#...
                                                  .......................#.#.........................#.....#....#......#.....#..#......#..............
                                                  ....###.........#......#..........#...............#......#...........#.....#....#.......#...........
                                                  ................#.#...#............#..#.........#..................#...................#...#........
                                                  ....#........#.............#..........................#...#......#.....#.#......#..............###..
                                                  ...#...........................#..#...................##.....##......#.#.......#..............#.....
                                                  ...#.............#...#..................#........#........#......#...#..................#....##.#...
                                                  .#...#.......##......#.....#................#..#.....#...........#.......#..........................
                                                  ..............#......##......................#.#.......#............................................
                                                  ...............................#........#..........................................#......#.........
                                                  ..#..#...........#..#.......#.#..........#......#.........#...#..................#.#.......#..#.#...
                                                  ........#......##.#......#.........#...#.........#................#...............#................#
                                                  ....#..........#..........................#..#................................#......#....#.........
                                                  ...#.............##........#......#.........#...................................#.......#......#.#..
                                                  ..#....#......##.#.............#................#......#....................................#......#
                                                  ...............................#........#.....#.#.............#.#......#........#.............##....
                                                  .......#..........#.........#..#..........#.............#.............................#..#..........
                                                  ......#....#...#.##.................#...............##..........................#................#..
                                                  ....#...........................#.....#....................#.......#.....................#..........
                                                  .....####........#.....#...#.....#.#.#.....#............................#.....................#.....
                                                  .....................##..................#..........#.........##......#....................#....#...
                                                  .......................#...........................................#......#.#.......................
                                                  ....#........#...................................#
                                                  ...#........#............................#..#...##
                                                  #.....#.............#..........#.......##...#..##.
                                                  .#.#.#..#......................#........#.....#...
                                                  ........................#..............#..........
                                                  .............................#................#...
                                                  .#........#.....................##.#.#.....#...#..
                                                  .....#.#....#.........#....#..............##...##.
                                                  .#...................#...#..........##............
                                                  ....#........##.......#.................#.....##..
                                                  ....#....##.....#...#..#..##....................#.
                                                  .............................##......#............
                                                  ..............#.................#.............#...
                                                  .......#...#...........#..#....#........#...##....
                                                  .......................#......#...........#...#...
                                                  ........................#.........#.............#.
                                                  .....#...........#................................
                                                  ....#..........#.#....#..#........................
                                                  ......#.....#..##.....###.................#.......
                                                  .........................#........#.......#.....#.
                                                  ........###..##.##.........#...........##....#....
                                                  ....#...#................##.#..........#.#........
                                                  ...........#...........#..#...#.......###.........
                                                  .........................#.#...#.....#....#.#.....
                                                  ..##..#............#...........#.....#.#..........
                                                  ..#...........#..#...........#...#.......#.....#..
                                                  #.......#.........#.....#.....#......#....#..#....
                                                  ..#..#.....#.#.................................#..
                                                  .............#.................#..#...............
                                                  ....#..............#............#.................
                                                  ..#.#......###..........##........#...............
                                                  .......#.............#...#........................
                                                  .#.......#..........................#.....##......
                                                  ...........#.....#...#.............#..............
                                                  #...........#.................##......#..##.......
                                                  .##....#..............#........#...........#......
                                                  .#...........#.#...#..#.......#.....#.............
                                                  .........#............#...............#...........
                                                  ....#.....................#..................#....
                                                  .#........#......#...........................#..#.
                                                  ..................#...#..........#...............#
                                                  .....................#..................#.........
                                                  ............#.....#.....#.........................
                                                  .........#........................##.............#
                                                  .#..#...........##.........#.....#....#.....#.....
                                                  .................................#....##..........
                                                  ..##.............#.....#..#...#...........#...#...
                                                  ..............................#.#.........#.......
                                                  ..................#......#..#.....................
                                                  .........#.........#...#.....#..#...........#....#
#.........................#......#...........#..##.#................#.................#...#.........
.................#............................#..............#...#..#.........#....#.......#.#.#....
##................##.......................#......................#.........#.......................
.........#......................#...........#...#......#..#........#...#....##........#.#......#....
..............#.......#........#..#...#........#........#..#........#.............#.#.#......#...#.#
.#.....#...#..............#.#.............#.#...#.....................#....#............#...........
...#................#..........#.............##.........................#.........#..#.........##...
.....................................##.#...#.#................#.................#..................
...........................#........#...................#.............#......#............#.#.......
....#.................#....#..#......................#.............#................................
........##.....#......#.#...........##................#....................#........#......#.....##.
#...................#.................................#....................#...#......#....#..##....
...................#....##..#.................#........#.........##.....................#...#....#..
..........#...#..............#.......#................#...........#..........#..#..#..#.........#...
.............#..#....#.....#......#...........#...........#........#.......#..#.#.##.....##......#..
....#.....#..#..............#...............#.......#.....................#.................#.......
...........#..........................#.........#.#....#.............................#.....#........
..............#........#....#.........#........#..#........#....#................#................#.
.....#............#.............#.........................................#.....##....#..........#..
.........#..........#...............................#..#...........#...#....#.................#.....
..#...........#.....................#....#..........................#.........#.....................
..............#.#....#.......#.....#..#.................#............#......#..................#.#..
...#...............#........#............#....#........#..........##.#....##...........##...........
.....#......#............#.........#.........#............#..............................#..........
...........#.#........#....#.....#.............................................#....................
........#......#..............................#.#...................#.#...........#.#...............
.#...#............#.......#......#...#.#...#...#....#........#................##..#.................
...........................#..........................#.....#..................#.........#..........
#..#..............#.#...............#..........##..........##......................#....#...........
.#................#......#.................#..............#.................#.......................
............#............#...#.........#.#.............................#...................#.....#..
.......#.............#.....#..................#.......#..............#..............................
.....#.......#.#.#..#.....#.....#.#.#.......#.#.....##..........................#...................
......##.......##.....#....#........................#..#...........#............................#...
........#..##..#................#.............#..........#..#.....................#..............#..
........#..........#..................##............................#......#..................#...#.
............#..................#...........#....#....#.............#.....................#...#..#...
....#.............##.#.........#.#......#.#.##..............#.....#......#.............#....#......#
......#.............#.......#.....................#.....#......#..#.................................
...#................#....#.#............................#...#..#...........#............#..#........
......#...................#...#......#........#.........#.........#..........#......#...#.......#...
.#.....#...................#.......#.....##......#................##.....##.#......#................
...#............#.........................#...................#...#..........#.#...#..#.#.......#...
......##............#.#............#..#............#.......................#........#.#...#....#..#.
.............................................................#.....................#....#.##........
..#................#..#....#....#.........................#.#.....#......................#....#.#...
.........................#...#.....................#.........#..........#.#......#................#.
..#.#...#........#....................##.............#...............#.....#.................#......
........#.......#.#...........#.................#....#..............#..........#.#.#................
........#.#.###............##............#........#...........................#...#.................
........................................#.........
..........##.#......#.............................
..............#........#.....#.#.#..............#.
....#...........#.#.#................#..#.......#.
..#...#.##...........#............................
..#.....#..#............#....................#....
.........#....#................................#..
............#.......#.......#...............#.....
............................#...#......##.........
......#.#...............#.........................
...............#...............#............#.#...
...#.#.........#....#.....#......#................
......#.....#.....................................
..........#....#....#..#.....#..............#.....
.............##.................#.................
...#..#.#................#..................#.....
........#.#.#....#......................#.........
..#................................#..#.......#...
#..........#..##.................#................
.#..........#..##...............##....#...........
..................##...........#.................#
..#..........#...........................#........
.......##...#.#...#.#...........###...............
.....#..#...#....#..#.....#................#......
..#........#......................................
..#.............##................#...#...........
............#....#..........#.#...................
.......#............#..#.....#.......##.......#...
............................#.....................
...#..#......#..................#.................
...............#......#...#...#...........#.......
......#...........................#...#.#..##.##..
.....#..........#..........................#......
...#...........##..#.........................#....
...#........#................#.............#....#.
.......##.......##.#......#................#......
.#........#........#...........#...............#..
..#............................#..#...............
...........................#.......#.....#........
.................#....##.....#..#..#....##..##....
.....................#.............#.....#........
..#..........................................#.#..
.........#..#.#..#...#................#..#.#......
..................#...#.#.......#.................
...#.........#.......#...###...........##...#.....
........................................#.........
......##......#............#.....#.......#..#.###.
#.....................#.......#.....#.............
...##..#..........#........#.....#.#......#.......
...#..#..#......#...#.............#.......#...#...

`
