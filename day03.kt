...
fun score(c: Char): Int {
    val char = c.toInt()
	return when (c) {
        in 'a'..'z' -> char - 'a'.toInt() + 1
        in 'A'..'Z' -> char - 'A'.toInt() + 1 + 26
...
fun part1(input: String): Int? {
	return input.split("\n").map{
        val lhs = it.slice(0..it.length/2-1).toSet()
		val rhs = it.slice(it.length/2..it.length-1).toSet()
        val intersection = lhs intersect rhs;
        score(intersection.elementAt(0))
    }.sum()
}
...
fun part2(input: String): Int? {
	return input.split("\n").chunked(3).map{
        val (a, b, c) = it
        val intersection = a.toSet() intersect b.toSet() intersect c.toSet();
        score(intersection.elementAt(0))
    }.sum()
}
...
val input2 = """BdbzzddChsWrRFbzBrszbhWMLNJHLLLLHZtSLglFNZHLJH
